...
final class AudioPlayerTests: XCTestCase {
  ...
  func testInitWithoutStreamsDoesNotSelectAnything() {
    let player = AudioPlayer()

    XCTAssertTrue(player.streams.isEmpty)
  ...
  func testAudioDelegateForwardsToUnderlyingMediaPlayer() {
    let player = AudioPlayer(streams: [stream(id: "1")])
    let spy = DelegateSpy()
    ...
    XCTAssertTrue(player.audioDelegate === player.delegate)
    XCTAssertTrue(player.mediaPlayer.delegate === spy)
  }

  func testMediaPlayerIsExposedForFineGrainedControl() {
    // The composition redesign makes the wrapped player explicitly
    // available so callers can reach into it (e.g. to attach an
    // `AVPlayerLayer`) instead of accidentally depending on
    // `AudioPlayer` inheriting the full `AVPlayer` API.
    let player = AudioPlayer(streams: [stream(id: "1")])
    XCTAssertNotNil(player.mediaPlayer.avPlayer)
  }
}
